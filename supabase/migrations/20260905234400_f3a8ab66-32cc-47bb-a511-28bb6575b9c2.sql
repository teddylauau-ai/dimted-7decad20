DROP FUNCTION IF EXISTS public.staff_grant_currency(uuid, integer, integer);
CREATE FUNCTION public.staff_grant_currency(_user_id uuid, _xp integer, _sparks integer)
...
BEGIN
  ...
  -- Levels are owner territory: only the owner may add or remove XP.
  IF coalesce(_xp, 0) <> 0 AND v_rank < 40 THEN
...
END;
$$;
REVOKE EXECUTE ON FUNCTION public.staff_grant_currency(uuid, integer, integer) FROM public, anon;

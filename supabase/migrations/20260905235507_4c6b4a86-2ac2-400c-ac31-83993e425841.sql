...
BEGIN
  ...
  UPDATE public.crews SET
    ...
    text_effect = CASE WHEN _patch->>'text_effect' IN ('none','glow','shimmer','sharp','soft','wave','pulse','prism','neon','chrome','gradient','glitch','flare') THEN _patch->>'text_effect' ELSE text_effect END,
    chat_bg = CASE WHEN _patch->>'chat_bg' IN ('none','grid','aurora','stars','waves','circuit','glass','nebula','eclipse','sovereign','ember','hex','matrix','bloom','void','prism') THEN _patch->>'chat_bg' ELSE chat_bg END
  WHERE id = _crew_id;
